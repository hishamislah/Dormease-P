  ...
  Widget build(BuildContext context) {
    return Consumer<DataProvider>(
      builder: (context, dataProvider, child) {
        ...
        final totalBeds =
            dataProvider.rooms.fold(0, (sum, room) => sum + room.totalBeds);
        final occupiedBeds =
            dataProvider.rooms.fold(0, (sum, room) => sum + room.occupiedBeds);
        final vacantBeds = totalBeds - occupiedBeds;
        final totalTenants = dataProvider.tenants.length;
        final tenantsUnderNotice =
            dataProvider.tenants.where((t) => t.underNotice).length;
        final tenantsWithRentDue =
            dataProvider.tenants.where((t) => t.rentDue).length;
        final activeTickets =
            dataProvider.tickets.where((t) => t.status != 'Closed').length;

        // Calculate revenue
        final totalRevenue = dataProvider.tenants
            .fold(0.0, (sum, tenant) => sum + tenant.monthlyRent);
        final paidPercentage = tenantsWithRentDue == 0
            ? 100.0
  ...
  Widget build(BuildContext context) {
    return Consumer<DataProvider>(
      builder: (context, dataProvider, child) {
        ...
        // Build list of tenants with their overdue/upcoming payment info
        List<({Tenant tenant, payment_service.PaymentRecord payment})>
            filteredEntries = [];

        for (final tenant in tenants) {
          // Generate payment history to check actual payment status
          final paymentHistory =
              payment_service.PaymentService.generatePaymentHistory(tenant);

          if (showUpcoming) {
            // Upcoming: find pending payments due within next 7 days
            final upcomingPayment = paymentHistory.where((p) {
              if (p.status != payment_service.PaymentStatus.pending)
                return false;
              final daysUntilDue = p.dueDate.difference(now).inDays;
              return daysUntilDue >= 0 && daysUntilDue <= 7;
            }).firstOrNull;

            if (upcomingPayment != null) {
              filteredEntries.add((tenant: tenant, payment: upcomingPayment));
            }
          } else {
            // Overdue: find the oldest overdue payment for this tenant
            final overduePayments = paymentHistory
                .where((p) => p.status == payment_service.PaymentStatus.overdue)
                .toList();

            if (overduePayments.isNotEmpty) {
              // Sort by due date to get the oldest overdue
              overduePayments.sort((a, b) => a.dueDate.compareTo(b.dueDate));
              filteredEntries
                  .add((tenant: tenant, payment: overduePayments.first));
            }
          }
        }
        ...
        return Container(
          decoration: BoxDecoration(
            color: Colors.white,
            border: Border.all(
                width: 0.5, color: showUpcoming ? Colors.orange : Colors.red),
            borderRadius: BorderRadius.circular(16),
          ),
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.payment,
                        color: showUpcoming ? Colors.orange : Colors.red),
                    const SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        "Payment Status",
                ...
                if (filteredEntries.isEmpty)
                  ...
                  ...filteredEntries.map((entry) {
                    ...
                    final badgeColor =
                        showUpcoming ? Colors.orange : Colors.red;
                    final bgColor = showUpcoming
                    ...
                    if (showUpcoming) {
                      final daysUntilDue =
                          payment.dueDate.difference(now).inDays;
                      badgeText =
                          daysUntilDue == 0 ? 'TODAY' : '$daysUntilDue\nDAYS';
                    } else {
                      final daysOverdue =
                          now.difference(payment.dueDate).inDays;
                      badgeText =
                          daysOverdue == 0 ? 'TODAY' : '$daysOverdue\nDAYS';
                    }
                ...
                children: [
                    ...
                    child: PieChart(PieChartData(
                      ...
                      sections: [
                        PieChartSectionData(
                            ...
                            radius: 30,
                            title:
                                "${data['paidRentPercentage'].toStringAsFixed(1)}%",
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          final tenantsWithDue = dataProvider.tenants
                              .where((t) => t.rentDue)
                              .toList();
                              ...
                              SnackBar(
                                content: Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 8),
                                    Text(
                                        "Rent reminders sent to ${tenantsWithDue.length} tenants!",
                                        style: const TextStyle(
                                            fontWeight: FontWeight.w500)),
                                  ],
                                ),
